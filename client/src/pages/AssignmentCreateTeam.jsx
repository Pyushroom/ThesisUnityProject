...
const AssignmentCreateTeam = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [quizzesResponse, exercisesResponse, teamResponse] = await Promise.all([
          ...
          axios.get(`http://localhost:5000/server/users/${currentUser.id}/teams`)
        ]);
        console.log(quizzesResponse.data)
        setQuizzes(quizzesResponse.data);
        setExercises(exercisesResponse.data);
        setTeams(exercisesResponse.data);
      } catch (error) {
        console.log(error);
      }
    };
  ...
  const handleCreate = () => {
    ...
    axios.post(
      `http://localhost:5000/server/users/${currentUser.id}/assigments/createAssigment`, newAssignment)
      .then(response => {
  ...
  return (
    ...
    <Grid item xs={12} sm={6}>
      ...
      {selectedTeam.id &&
        <Box sx={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
          <Typography variant="subtitle1">Selected Team:</Typography>
            <Typography variant="subtitle1" className={classes.redText}>{selectedTeam.name}</Typography>
        </Box>
      }
      <AssigmentMembersTable teamId={selectedTeam.id} onSelectUsers={handleSelectUsers} />
    </Grid>
  </Grid>
...
export default AssignmentCreateTeam;
